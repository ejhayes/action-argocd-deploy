...
class ArgoApplication {
    constructor(releaseName, args) {
        this.releaseName = releaseName;
        this.args = args;
    }
    toManifest(replaceTokens = true) {
        return {
            metadata: {
            ...
            spec: {
                ...
                source: {
                    ...
                    helm: {
                        values: (0, js_yaml_1.dump)((0, js_yaml_1.load)(replaceTokens
                            ? this.replaceTokens(this.args.helmValues.toString(), this.args.tokens)
                            : this.args.helmValues.toString()), { noCompatMode: true }),
                    },
                },
    ...
    replaceTokens(str, tokens, tokenPrefix = '__', tokenSuffix = '__') {
        if (!tokens) {
            return str;
        }
        ...
        if (missingTokens.size > 0) {
            throw new Error(`Missing token values for: ${Array.from(missingTokens).join(', ')}`);
        }
        return ret;
    }
}
...
const core_1 = __nccwpck_require__(2186);
const typed_rest_client_1 = __nccwpck_require__(7405);
const application_1 = __nccwpck_require__(1618);
class ArgoCDApi {
    constructor(_args, dryRun = false) {
        this._args = _args;
        this.dryRun = dryRun;
        this._client = new typed_rest_client_1.RestClient('gha-argocd-deploy/v1', `${_args.baseUrl}/api/v1`, [], { headers: {} });
        if (_args.accessToken !== '') {
            this._token = _args.accessToken;
        }
    }
    createApplication(releaseName, args) {
        return __awaiter(this, void 0, void 0, function* () {
            const app = new application_1.ArgoApplication(releaseName, args);
            if (this.dryRun)
                return app;
            const appExists = yield (yield this._getClient()).get(`applications/${releaseName}`);
            let res;
            (0, core_1.info)(`Tokenizing with: ${JSON.stringify(args.tokens, null, 2)}`);
            if (appExists.statusCode === 200) {
                (0, core_1.info)(`Updating application: ${releaseName}`);
                res = yield (yield this._getClient()).replace(`applications/${releaseName}`, app.toManifest());
            }
            else {
                (0, core_1.info)(`Creating application: ${releaseName}`);
                res = yield (yield this._getClient()).create(`applications`, app.toManifest());
            }
            if (res.statusCode === 200) {
                return app;
            }
    ...
    destroyApplication(releaseName) {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.dryRun)
                return;
            const appRes = yield (yield this._getClient()).get(`applications/${releaseName}`);
            if (appRes.statusCode === 404) {
                return;
            }
            const deleteRes = yield (yield this._getClient()).del(`applications/${releaseName}`);
    ...
    _getToken() {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this._token) {
                const res = yield this._client.create('session', {
    ...
    _getClient() {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if ((_a = this._client.client.requestOptions) === null || _a === void 0 ? void 0 : _a.headers) {
...
const js_yaml_1 = __nccwpck_require__(1917);
const argocd_1 = __nccwpck_require__(5138);
var INPUTS;
(function (INPUTS) {
    INPUTS["ACCESS_TOKEN"] = "accessToken";
    INPUTS["ACTION"] = "action";
    INPUTS["ANNOTATIONS"] = "annotations";
    INPUTS["BASE_URL"] = "baseUrl";
    INPUTS["CLIENT_ID"] = "clientId";
    INPUTS["CLIENT_SECRET"] = "clientSecret";
    ...
    INPUTS["REPO_REVISION"] = "repoRevision";
    INPUTS["REPO_URL"] = "repoUrl";
    INPUTS["REPO_WEBURL"] = "repoWebUrl";
    INPUTS["TOKENS"] = "tokens";
    INPUTS["VALUES_FILE"] = "valuesFile";
})(INPUTS || (INPUTS = {}));
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        switch (args.action) {
            case 'delete':
                return yield deleteApplication(args);
            case 'upsert':
                return yield upsertApplication(args);
            default:
...
function upsertApplication(args) {
    return __awaiter(this, void 0, void 0, function* () {
        const client = getClient(args);
        const app = yield client.createApplication(args.name, {
            clusterName: args.clusterName,
            gitRef: args.prNumber ? `pull/${args.prNumber}/head` : args.repoRevision,
            gitRepo: args.repoUrl,
            helmValues: yield (0, util_1.promisify)(fs_1.readFile)(args.valuesFile),
        ...
        try {
            yield app.toManifest();
        }
        catch (err) {
            core.error(err);
            throw err;
        }
        const manifestDiff = (0, diff_1.createTwoFilesPatch)(`${args.valuesFile}.original`, `${args.valuesFile}.tokenized`, (0, js_yaml_1.dump)(app.toManifest(false)), (0, js_yaml_1.dump)(app.toManifest()));
        core.summary.addRaw(`# ${args.dryRun
...
### full spec
\`\`\`yaml
${(0, js_yaml_1.dump)(app.toManifest())}
\`\`\`
...
function deleteApplication(args) {
    return __awaiter(this, void 0, void 0, function* () {
        const client = getClient(args);
        yield client.destroyApplication(args.name);
        core.summary.addRaw(`# ${args.dryRun
            ? ':construction: :construction: DRY RUN :construction: :construction:'
            : ''} ArgoCD Actions Summary 
...
function getInputs() {
    ...
    const inputs = {
        ...
        annotations: ((0, js_yaml_1.load)(core.getInput(INPUTS.ANNOTATIONS, { required: false })) || {}),
        ...
        info: ((0, js_yaml_1.load)(core.getInput(INPUTS.INFO, { required: false })) ||
            {}),
        labels: ((0, js_yaml_1.load)(core.getInput(INPUTS.LABELS, { required: true })) ||
            {}),
        path: core.getInput(INPUTS.PATH, { required: true }),
        tokens: ((0, js_yaml_1.load)(core.getInput(INPUTS.TOKENS, { required: true })) ||
            {}),
    };
    const accessTokenExists = inputs.accessToken.trim() !== '';
    const clientCredentialsExists = inputs.clientId.trim() !== '' && inputs.clientSecret.trim() !== '';
    if (accessTokenExists && clientCredentialsExists) {
        core.setFailed('You must specify access token OR client credentials, not both');
