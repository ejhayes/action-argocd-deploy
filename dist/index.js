...
const application_1 = __nccwpck_require__(6970);
class ArgoCDApi {
    ...
    async createApplication(releaseName, args) {
        const app = new application_1.ArgoApplication(releaseName, args);
        if (this.dryRun)
            return app;
        const appExists = await (await this._getClient()).get(`applications/${releaseName}`);
        let res;
        ...
        if (appExists.statusCode === 200) {
            ...
            res = await (await this._getClient()).replace(`applications/${releaseName}`, app.toManifest());
        }
        else {
            (0, core_1.info)(`Creating application: ${releaseName}`);
            res = await (await this._getClient()).create(`applications`, app.toManifest());
        }
    ...
    async destroyApplication(releaseName) {
        if (this.dryRun)
            return;
        const appRes = await (await this._getClient()).get(`applications/${releaseName}`);
        if (appRes.statusCode === 404) {
            return;
        }
        const deleteRes = await (await this._getClient()).del(`applications/${releaseName}`);
    ...
    async _getToken() {
        if (!this._token) {
            const res = await this._client.create('session', {
...
const js_yaml_1 = __nccwpck_require__(1917);
const argocd_1 = __nccwpck_require__(6230);
var INPUTS;
(function (INPUTS) {
...
function getInputs() {
    return {
        ...
        gitRepoUrl: `${process.env.GITHUB_SERVER_URL.replace(/^https:/, 'git:')}/${process.env.GITHUB_REPOSITORY.replace(/(?<!\.git)$/, '.git')}`,
